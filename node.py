...
class Node:
    def __init__(
        ...
        logging.info(
            f"Ensured shards directory exists at: {os.path.abspath(self.shards_dir)}"
        )
        logging.info(
            f"Ensured master data directory exists at: {os.path.abspath(MASTER_DATA_DIR)}"
        )
        logging.info(
            f"Ensured master retrieved shards directory exists at: {os.path.abspath(MASTER_RETRIEVED_SHARDS_DIR)}"
        )
        ...
        logging.info(
            f"[{self.address}] Starting as {self.role.upper()}. Explicit master: {master_address}"
        )
        ...
        if self.role == "worker" and self.current_master_address:
            logging.info(
                f"[{self.address}] Creating/Updating MasterService stubs for {self.current_master_address}"
            )
        ...
        logging.info(
            f"[{self.address}] Initialized as {self.role.upper()}. Master is {self.current_master_address}. My ID: {self.id}. Current Term: {self.current_term}"
        )

    def _get_or_create_channel(self, node_address: str) -> grpc.aio.Channel:
        if node_address not in self._channels or (
            self._channels.get(node_address)
            and self._channels[node_address]._channel.closed()
        ):
            logging.info(
                f"[{self.address}] Creating new channel for {node_address} with max message size {MAX_GRPC_MESSAGE_LENGTH} bytes"
            )
    ...
    def _create_stubs_for_node(self, node_address: str):
        ...
        if self.role == "master":
            self._worker_stubs[node_address] = replication_pb2_grpc.WorkerServiceStub(
                channel
            )
            logging.info(
                f"[{self.address}] Created WorkerService stub for {node_address}"
            )

    def _create_master_stubs(self, master_address: str):
        master_channel_valid = (
            self._master_channel
            and self._master_channel_address == master_address
            and not self._master_channel._channel.closed()
        )
    ...
    def _get_or_create_master_stub(
        self,
    ) -> Optional[replication_pb2_grpc.MasterServiceStub]:
    ...
    async def start(self):
        ...
        logging.info(
            f"[{self.address}] Server starting at {self.address} as {self.role.upper()} with max message size {MAX_GRPC_MESSAGE_LENGTH} bytes"
        )
            ...
            for task in done:
                try:
                    ...
                    if is_master and term >= highest_term_found:
                        ...
                        logging.info(
                            f"[{self.address}] Discovered potential master at {node_addr} with term {term}."
                        )
                except Exception as e:
                    logging.error(
                        f"[{self.address}] Error processing discovery task result: {type(e).__name__} - {e}"
                    )
        ...
        if discovered_master_address and highest_term_found >= self.current_term:
            logging.info(
                f"[{self.address}] Discovered active master {discovered_master_address} with term {highest_term_found}. Transitioning to follower state."
            )
            self.state = "follower"
            self.role = "worker"
            ...
            logging.info(
                f"[{self.address}] Ensuring master stubs are created for {self.current_master_address} and starting health check routine."
            )
            self._create_master_stubs(self.current_master_address)
            self._master_health_check_task = asyncio.create_task(
            ...
            logging.info(
                f"[{self.address}] No active master found with term >= my current term during startup discovery. Proceeding with initial role."
            )
            ...
            if self.role == "master":
                logging.info(
                    f"[{self.address}] Initializing worker stubs based on known nodes."
                )
                self._worker_stubs = {}
                for node_addr in self.known_nodes:
                    if node_addr != self.address:
    ...
    async def _query_node_for_master(
        self, node_stub: replication_pb2_grpc.NodeServiceStub, node_address: str
    ) -> Tuple[str, bool, int]:
        """Queries a node for its master status and term."""
        try:
            logging.debug(
                f"[{self.address}] Checking node {node_address} for master status."
            )
            response = await asyncio.wait_for(
                node_stub.GetNodeStats(replication_pb2.NodeStatsRequest()), timeout=2
            )
            logging.debug(
                f"[{self.address}] Received stats from {node_address}. Is Master: {response.is_master}, Term: {response.current_term}"
            )
            return node_address, response.is_master, response.current_term
        except (grpc.aio.AioRpcError, asyncio.TimeoutError) as e:
            logging.debug(
                f"[{self.address}] Node {node_address} unresponsive during startup discovery: {e}"
            )
            return node_address, False, -1
        except Exception as e:
            logging.error(
                f"[{self.address}] Unexpected error during startup discovery check for {node_address}: {type(e).__name__} - {e}",
                exc_info=True,
            )
            return node_address, False, -1
    ...
    async def stop(self):
        ...
        logging.info(
            f"[{self.address}] Cancelling {len(self._background_tasks)} background tasks."
        )
        ...
        logging.info(
            f"[{self.address}] Cancelling {len(processing_task_list)} processing tasks."
        )
        ...
        if self._master_channel and not self._master_channel._channel.closed():
            logging.info(
                f"[{self.address}] Closing master channel to {self._master_channel_address}"
            )
            channel_close_tasks.append(
                asyncio.create_task(self._master_channel.close())
            )
    ...
    async def AnnounceMaster(
        self,
        request: replication_pb2.MasterAnnouncement,
        context: grpc.aio.ServicerContext,
    ) -> replication_pb2.MasterAnnouncementResponse:
        """Handles incoming MasterAnnouncement RPCs."""
        logging.info(
            f"[{self.address}] Received MasterAnnouncement from {context.peer()}. New master is {request.master_address} (ID: {request.node_id_of_master}, Term: {request.term})"
        )

        if request.term > self.current_term:
            logging.info(
                f"[{self.address}] Received MasterAnnouncement with higher term ({request.term} > {self.current_term}). Updating term and reverting to follower."
            )
            ...
            logging.info(
                f"[{self.address}] Updating Master stubs to point to {request.master_address} and attempting to report unreported shards."
            )
            ...
            if self._election_task and not self._election_task.done():
                logging.info(
                    f"[{self.address}] Cancelling pending election task due to new master announcement."
                )
                self._election_task.cancel()
                self._election_task = None
            if (
                self._pre_election_delay_task
                and not self._pre_election_delay_task.done()
            ):
                logging.info(
                    f"[{self.address}] Cancelling pending pre-election delay due to new master announcement."
                )
                self._pre_election_delay_task.cancel()
                self._pre_election_delay_task = None
            if (
                self._master_announcement_task
                and not self._master_announcement_task.done()
            ):
            ...
            if (
                self._other_nodes_health_check_task
                and not self._other_nodes_health_check_task.done()
            ):
                logging.info(
                    f"[{self.address}] Cancelling other nodes health check task."
                )
                self._other_nodes_health_check_task.cancel()
                self._other_nodes_health_check_task = None

            if self.role == "worker" and (
                self._master_health_check_task is None
                or self._master_health_check_task.done()
            ):
            ...
            if self.state == "follower":
                logging.info(
                    f"[{self.address}] Received MasterAnnouncement with equal term ({request.term}) while in follower state. Accepting as heartbeat."
                )
                self.current_master_address = request.master_address
                self.leader_address = request.master_address
                self.last_heartbeat_time = time.monotonic()
                ...
                logging.warning(
                    f"[{self.address}] Received MasterAnnouncement with equal term ({request.term}) while in candidate state. Stepping down to follower."
                )
                self.state = "follower"
                self.voted_for = None
                self.current_master_address = request.master_address
                ...
                if (
                    ...
                    logging.info(
                        f"[{self.address}] Cancelling pending pre-election delay due to stepping down from candidate."
                    )
                    self._pre_election_delay_task.cancel()
                    self._pre_election_delay_task = None
                if self._election_task and not self._election_task.done():
                ...
                if self.role == "worker" and (
                    ...
                    logging.info(
                        f"[{self.address}] Starting master health check routine."
                    )
                    self._master_health_check_task = asyncio.create_task(
                        self.check_master_health()
                    )
                ...
                logging.warning(
                    f"[{self.address}] Received MasterAnnouncement with equal term ({request.term}) while in leader state. Potential split-brain. Ignoring announcement and continuing as leader."
                )

        elif request.term < self.current_term:
            if self.current_master_address is not None:
                logging.info(
                    f"[{self.address}] Received MasterAnnouncement with lower term ({request.term} < {self.current_term}). Ignoring."
                )
            else:
                logging.warning(
                    f"[{self.address}] Received MasterAnnouncement with lower term ({request.term} < {self.current_term}) but I don't know a master. Accepting for now to discover a master."
                )
                self.current_term = request.term
                self.state = "follower"
                self.voted_for = None
                ...
                if (
                    ...
                    logging.info(
                        f"[{self.address}] Cancelling pending pre-election delay due to accepting lower term announcement (no master known)."
                    )
                    self._pre_election_delay_task.cancel()
                    self._pre_election_delay_task = None
        ...
        response = replication_pb2.MasterAnnouncementResponse(
            status=f"Acknowledged by {self.id}", node_id=self.id
        )
        return response

    async def RequestVote(
        self, request: replication_pb2.VoteRequest, context: grpc.aio.ServicerContext
    ) -> replication_pb2.VoteResponse:
        """Handles incoming VoteRequest RPCs with improved tiebreaking."""
        logging.info(
            f"[{self.address}] Received VoteRequest from {request.candidate_id} with term {request.term} and score {request.score}"
        )

        if request.term < self.current_term:
            logging.info(
                f"[{self.address}] Rejecting vote: candidate term {request.term} < our term {self.current_term}"
            )
            return replication_pb2.VoteResponse(
                term=self.current_term, vote_granted=False, voter_id=self.address
            )

        if request.term > self.current_term:
            logging.info(
                f"[{self.address}] Candidate has higher term {request.term} > {self.current_term}, updating term"
            )
            self.current_term = request.term
            self.state = "follower"
            self.voted_for = None
        ...
        if (
            ...
            if request.score < my_score:
                vote_granted = True
                logging.info(
                    f"[{self.address}] Granting vote: candidate score {request.score} < our score {my_score}"
                )
                ...
                if request.candidate_id < self.address:
                    vote_granted = True
                    logging.info(
                        f"[{self.address}] Granting vote: tied score but candidate ID {request.candidate_id} < our ID {self.address}"
                    )
                else:
                    logging.info(
                        f"[{self.address}] Rejecting vote: tied score but candidate ID {request.candidate_id} >= our ID {self.address}"
                    )
            else:
                logging.info(
                    f"[{self.address}] Rejecting vote: candidate score {request.score} > our score {my_score}"
                )
            ...
            vote_granted = False
            logging.info(
                f"[{self.address}] Already voted for {self.voted_for} in term {self.current_term}, rejecting"
            )

        return replication_pb2.VoteResponse(
            term=self.current_term, vote_granted=vote_granted, voter_id=self.address
        )
    ...
    def reset_election_timer(self):
        ...
        logging.info(
            f"[{self.address}] New election timeout: {self.election_timeout:.2f}s (attempt {self.election_attempts})"
        )
        self.last_heartbeat_time = time.monotonic()

    async def GetNodeStats(
        self,
        request: replication_pb2.NodeStatsRequest,
        context: grpc.aio.ServicerContext,
    ) -> replication_pb2.NodeStatsResponse:
        """Provides statistics about the node."""
        logging.debug(
            f"[{self.address}] Received GetNodeStats request from {context.peer()}"
        )
    ...
    async def GetCurrentMaster(
        self,
        request: replication_pb2.GetCurrentMasterRequest,
        context: grpc.aio.ServicerContext,
    ) -> replication_pb2.GetCurrentMasterResponse:
        """Provides the address and term of the current master."""
        logging.debug(
            f"[{self.address}] Received GetCurrentMaster request from {context.peer()}"
        )
        return replication_pb2.GetCurrentMasterResponse(
            master_address=self.current_master_address
            if self.current_master_address
            else "",
    ...
    def calculate_server_score(self, force_fresh=False):
        ...
        if (
            not force_fresh
            and self.score_valid
            and (time.monotonic() - self.score_last_updated)
            < self.score_update_interval
        ):
    ...
    async def ReportResourceScore(
        self,
        request: replication_pb2.ReportResourceScoreRequest,
        context: grpc.aio.ServicerContext,
    ) -> replication_pb2.ReportResourceScoreResponse:
        """Handles incoming resource scores from workers."""
        if self.role != "master":
            logging.info(
                f"[{self.address}] Received score report but I'm not master. Informing worker."
            )
            return replication_pb2.ReportResourceScoreResponse(
                success=False, message="Not master"
            )
        ...
        logging.info(
            f"[{self.address}] Received resource score from {worker_address}: {score.score}"
        )
    ...
    async def calculate_and_send_score_to_master(self):
        if not self.current_master_address or not self._validate_stub(
        ...
        if (
            not self.current_master_address
            or self.current_master_address == self.address
            or self._pre_election_delay_task is not None
            or self.state == "candidate"
        ):
            logging.debug(
                f"[{self.address}] Skipping score report: No master available, I am the master, or election in progress"
            )
            return
        ...
        if not master_node_stub:
            logging.warning(
                f"[{self.address}] No stub for master at {self.current_master_address}"
            )
            return

        try:
            request = replication_pb2.ReportResourceScoreRequest(
                worker_address=self.address, resource_score=resource_score
            )
    ...
    async def _update_score_periodically(self):
        ...
        while not getattr(self, "_shutdown_flag", False):
            self.calculate_server_score(force_fresh=True)
            logging.debug(
                f"[{self.address}] Updated score: {self.current_score['score']}"
            )
    ...
    async def RegisterNode(
        self,
        request: replication_pb2.RegisterNodeRequest,
        context: grpc.aio.ServicerContext,
    ) -> replication_pb2.RegisterNodeResponse:
        """Handles registration of new nodes in the network."""
        node_addr = f"{request.address}:{request.port}"
        node_id = request.node_id

        logging.info(
            f"[{self.address}] Received RegisterNode request from {node_id} at {node_addr}"
        )
    ...
    async def UpdateNodeList(
        self,
        request: replication_pb2.UpdateNodeListRequest,
        context: grpc.aio.ServicerContext,
    ) -> replication_pb2.UpdateNodeListResponse:
        """Updates this node's knowledge of the network topology."""
        logging.info(
            f"[{self.address}] Received UpdateNodeList with {len(request.node_addresses)} nodes"
        )
        ...
        for node_addr in request.node_addresses:
            if node_addr != self.address and node_addr not in self.known_nodes:
                logging.info(
                    f"[{self.address}] Adding new node {node_addr} to known_nodes"
                )
                self.known_nodes.append(node_addr)
                self._create_stubs_for_node(node_addr)
                updated = True
        ...
        if (
            ...
            and request.master_address != self.current_master_address
        ):
            logging.info(
                f"[{self.address}] Updating master address from {self.current_master_address} to {request.master_address}"
            )
            self.current_master_address = request.master_address
            self.leader_address = request.master_address
            updated = True
    ...
    async def broadcast_node_list(self):
        ...
        logging.info(
            f"[{self.address}] Broadcasting node list to all nodes: {len(self.known_nodes)} nodes"
        )
        ...
        request = replication_pb2.UpdateNodeListRequest(
            node_addresses=all_nodes, master_address=self.address
        )
        ...
        for node_addr in list(all_nodes):
            ...
            if not node_stub:
                logging.warning(
                    f"[{self.address}] No NodeService stub for {node_addr}, cannot update"
                )
                continue
            ...
            try:
                ...
                logging.debug(
                    f"[{self.address}] Successfully sent node list to {node_addr}"
                )
            except Exception as e:
                logging.error(
                    f"[{self.address}] Failed to send node list to {node_addr}: {e}"
                )

    async def GetAllNodes(
        self,
        request: replication_pb2.GetAllNodesRequest,
        context: grpc.aio.ServicerContext,
    ) -> replication_pb2.GetAllNodesResponse:
        ...
        for node_addr in self.known_nodes + [self.address]:
            ...
            node_infos.append(
                replication_pb2.NodeInfo(node_id=node_addr, address=host, port=port)
            )
    ...
    async def RegisterWorker(
        ...
        if worker_addr not in self.known_nodes:
            ...
            return replication_pb2.RegisterWorkerResponse(
                success=True, message=f"{worker_addr} registered"
            )
        else:
            return replication_pb2.RegisterWorkerResponse(
                success=False, message=f"{worker_addr} was already registered"
            )

    async def UploadVideo(
        self,
        request_iterator: AsyncIterator[replication_pb2.UploadVideoChunk],
        context: grpc.aio.ServicerContext,
    ) -> replication_pb2.UploadVideoResponse:
        if self.role != "master":
            return replication_pb2.UploadVideoResponse(
                success=False, message="This node is not the master."
            )
        ...
        try:
            ...
            if not first_chunk.is_first_chunk:
                raise ValueError(
                    "First chunk in UploadVideo stream must have is_first_chunk set to True."
                )

            video_id = first_chunk.video_id or str(uuid.uuid4())
            original_filename = first_chunk.original_filename or f"{video_id}"
            file_extension = (
                original_filename.split(".")[-1] if "." in original_filename else "mp4"
            )

            temp_input_path = os.path.join(
                MASTER_DATA_DIR, f"{video_id}_original.{file_extension}"
            )
            ...
            if hasattr(self, "backup_servers") and self.backup_servers:
                logging.info(
                    f"[{self.address}] Replicating original input file for video {video_id} to backup servers"
                )
                try:
                    ...
                    for backup_addr in self.backup_servers:
                        try:
                            ...
                            if response.success:
                                logging.info(
                                    f"[{self.address}] Successfully replicated original input file for video {video_id} to backup server {backup_addr}"
                                )
                            else:
                                logging.error(
                                    f"[{self.address}] Failed to replicate original input file for video {video_id} to backup server {backup_addr}: {response.message}"
                                )

                        except Exception as e:
                            logging.error(
                                f"[{self.address}] Error replicating original input file to backup server {backup_addr}: {e}"
                            )
                except Exception as e:
                    logging.error(
                        f"[{self.address}] Error reading original input file for replication: {e}"
                    )

            logging.info(
                f"[{self.address}] Finished receiving all chunks for video ID: {video_id}. File saved to {temp_input_path}"
            )
            ...
            output_pattern = os.path.join(
                MASTER_DATA_DIR, f"{video_id}_shard_%04d.{file_extension}"
            )
            ...
            try:
                ...
                shard_files = sorted(
                    glob.glob(
                        os.path.join(
                            MASTER_DATA_DIR, f"{video_id}_shard_*.{file_extension}"
                        )
                    )
                )
                ...
                return replication_pb2.UploadVideoResponse(
                    video_id=video_id,
                    success=False,
                    message=f"Segmentation failed: {e.stderr.decode()}",
                )
            except Exception as e:
                self.video_statuses[video_id]["status"] = "failed_segmentation"
                return replication_pb2.UploadVideoResponse(
                    video_id=video_id,
                    success=False,
                    message=f"Segmentation failed: {type(e).__name__} - {e}",
                )
    ...
    async def _distribute_shards(
        ...
        logging.info(
            f"[{self.address}] Starting bulk distribution of {len(shard_files)} shards"
        )
        ...
        for attempt in range(max_retries):
            valid_workers = [
                addr
                for addr in self._worker_stubs
                if self._worker_stubs.get(addr) is not None
            ]
            if valid_workers:
                break
            logging.warning(
                f"[{self.address}] No workers available. Retrying in {retry_delay}s (attempt {attempt + 1}/{max_retries})"
            )
            await asyncio.sleep(retry_delay)
        if not valid_workers:
            logging.error(
                f"[{self.address}] Aborting distribution: No workers after {max_retries} retries"
            )
            return
        ...
        #         video_id, "distributed", f"All {len(shard_files)} shards distributed"
        #     )
        # else:
        #     self._update_video_status(
        #         video_id,
    ...
    async def _send_shard_to_worker(
        self,
        worker: str,
        video_id: str,
        shard_file: str,
        ...
        try:
            ...
            if not worker_stub:
                logging.error(
                    f"[{self.address}] Worker stub for {worker} is missing or invalid. Cannot distribute {shard_file}."
                )
            ...
            if not channel:
                logging.error(
                    f"[{self.address}] Channel for worker {worker} not found in _channels. Cannot distribute {shard_file}."
                )
            ...
            if channel.get_state() == grpc.ChannelConnectivity.SHUTDOWN:
                logging.error(
                    f"[{self.address}] Channel to worker {worker} is shut down. Cannot distribute {shard_file}."
                )
            ...
            logging.info(
                f"[{self.address}] Attempting to send shard {os.path.basename(shard_file)} to {worker}"
            )
            await asyncio.wait_for(worker_stub.ProcessShard(request), timeout=30000)
            logging.info(
                f"[{self.address}] Successfully sent shard {os.path.basename(shard_file)} to {worker}"
            )

            if video_id in self.video_statuses:
                self.video_statuses[video_id]["shards"][
                ...
                logging.error(
                    f"[{self.address}] Video ID {video_id} missing in video_statuses during shard distribution."
                )
            ...
            logging.error(
                f"[{self.address}] RPC failed or timed out when distributing {shard_file} to {worker}: {type(e).__name__} - {e.details() if isinstance(e, grpc.aio.AioRpcError) else e}. Marking shard as failed distribution.",
                exc_info=True,
            )
            return False
        except Exception as e:
            logging.error(
                f"[{self.address}] Failed to distribute {shard_file} to {worker}: {type(e).__name__} - {e}. Marking shard as failed distribution.",
                exc_info=True,
            )
            return False
    ...
    async def ReportWorkerShardStatus(
        self,
        request: replication_pb2.ReportWorkerShardStatusRequest,
        context: grpc.aio.ServicerContext,
    ) -> replication_pb2.ReportWorkerShardStatusResponse:
        """Handles status updates from worker nodes (master only)."""
        if self.role != "master":
            return replication_pb2.ReportWorkerShardStatusResponse(
                success=False, message="This node is not the master."
            )
        ...
        logging.info(
            f"[{self.address}] Received ReportWorkerShardStatus for video {video_id}, shard {shard_id} from {worker_address} with status: {status}"
        )

        if video_id not in self.video_statuses:
            logging.warning(
                f"[{self.address}] Received status update for unknown video ID: {video_id}"
            )
            return replication_pb2.ReportWorkerShardStatusResponse(
                success=False, message=f"Unknown video ID: {video_id}"
            )

        if shard_id in self.video_statuses[video_id]["shards"] and self.video_statuses[
            video_id
        ]["shards"][shard_id]["status"] in [
            "failed_sending",
            "rpc_failed",
            "failed_distribution",
        ]:
            logging.info(
                f"[{self.address}] Received status for shard {shard_id} previously marked as failed distribution. Updating status."
            )
            original_index = self.video_statuses[video_id]["shards"][shard_id].get(
                "index", -1
            )
            ...
            logging.warning(
                f"[{self.address}] Received status update for unknown shard ID {shard_id} for video {video_id} that wasn't in the initial distribution list."
            )
            self.video_statuses[video_id]["shards"][shard_id] = {
                "status": status,
                "worker_address": worker_address,
            ...
            self.video_statuses[video_id]["shards"][shard_id]["worker_address"] = (
                worker_address
            )

        if status == "processed_successfully":
            retrieve_task = asyncio.create_task(
                self._retrieve_processed_shard(video_id, shard_id, worker_address)
            )
    ...
    async def _retrieve_processed_shard(
        self, video_id: str, shard_id: str, worker_address: str
    ):
        """Retrieves a processed shard from a worker node."""
        logging.info(
            f"[{self.address}] Requesting processed shard {shard_id} for video {video_id} from worker {worker_address}"
        )
        ...
        if not worker_stub:
            logging.error(
                f"[{self.address}] No WorkerService stub for {worker_address}. Cannot retrieve shard {shard_id}. Marking shard as retrieval failed."
            )
            if shard_id in self.video_statuses[video_id]["shards"]:
                self.video_statuses[video_id]["shards"][shard_id]["status"] = (
                    "retrieval_failed"
                )
                self.video_statuses[video_id]["shards"][shard_id]["message"] = (
        ...
        try:
            ...
            response = await asyncio.wait_for(
                worker_stub.RequestShard(request), timeout=30
            )

            if response.success:
                logging.info(
                    f"[{self.address}] Successfully retrieved processed shard {shard_id} from {worker_address}"
                )
                if (
                    video_id in self.video_statuses
                    and shard_id in self.video_statuses[video_id]["shards"]
                ):
                    self.video_statuses[video_id]["retrieved_shards"][shard_id] = {
                        "data": response.shard_data,
                        "index": self.video_statuses[video_id]["shards"][shard_id].get(
                            "index", -1
                        ),
                    }
                    self.video_statuses[video_id]["shards"][shard_id]["status"] = (
                    ...
                    total_successfully_processed_shards = sum(
                        1
                        for s in video_info["shards"].values()
                        if s["status"] in ["processed_successfully", "retrieved"]
                    )
                    retrieved_count = sum(
                        1
                        for s in video_info["shards"].values()
                    ...
                    logging.info(
                        f"[{self.address}] Video {video_id} — retrieved {retrieved_count}/{total_successfully_processed_shards} processed shards."
                    )
                    video_info = self.video_statuses[video_id]
                    ...
                    if retrieved_count == total_shards:
                        if (
                            video_info["concatenation_task"] is None
                            or video_info["concatenation_task"].done()
                        ):
                            logging.info(
                                f"All {total_shards} shards retrieved. Starting concatenation."
                            )
                            video_info["status"] = "concatenating"
                            concat_task = asyncio.create_task(
                                self._concatenate_shards(video_id)
                            )
                            self._background_tasks.append(concat_task)

                else:
                    logging.warning(
                        f"[{self.address}] Received processed shard {shard_id} for video {video_id} but video/shard info not found in status tracking. Dropping shard data."
                    )

            else:
                logging.error(
                    f"[{self.address}] Worker {worker_address} failed to provide shard {shard_id}: {response.message}. Marking shard as retrieval failed."
                )
                if shard_id in self.video_statuses[video_id]["shards"]:
                    self.video_statuses[video_id]["shards"][shard_id]["status"] = (
                        "retrieval_failed"
            ...
            logging.error(
                f"[{self.address}] RPC failed or timed out when retrieving shard {shard_id} from {worker_address}: {e.code()} - {e.details()}. Marking shard as retrieval failed.",
                exc_info=True,
            )
            if shard_id in self.video_statuses[video_id]["shards"]:
                self.video_statuses[video_id]["shards"][shard_id]["status"] = (
                ...
                self.video_statuses[video_id]["shards"][shard_id]["message"] = (
                    f"RPC failed or timed out: {type(e).__name__} - {e}"
                )
        except Exception as e:
            logging.error(
                f"[{self.address}] Failed to retrieve shard {shard_id} from {worker_address}: {type(e).__name__} - {e}",
                exc_info=True,
            )
            if shard_id in self.video_statuses[video_id]["shards"]:
                self.video_statuses[video_id]["shards"][shard_id]["status"] = (
    ...
    async def _concatenate_shards(self, video_id: str):
        ...
        try:
            ...
            replicate_task = asyncio.create_task(
                self._replicate_video_data(video_id, output_path)
            )
            self._background_tasks.append(replicate_task)

        except subprocess.CalledProcessError as e:
            logging.error(
                f"[{self.address}] FFmpeg failed (code {e.returncode}): {e.stderr}"
            )
    ...
    async def RetrieveVideo(
        self,
        request: replication_pb2.RetrieveVideoRequest,
        context: grpc.aio.ServicerContext,
    ) -> AsyncIterator[replication_pb2.RetrieveVideoChunk]:
        """Handles video retrieval requests (master only)."""
        if self.role != "master":
            logging.error(
                f"[{self.address}] RetrieveVideo request received by non-master node."
            )
            await context.abort(
                grpc.StatusCode.FAILED_PRECONDITION, "This node is not the master."
            )
            return

        video_id = request.video_id
        logging.info(
            f"[{self.address}] Received RetrieveVideo request for video ID: {video_id}"
        )
        ...
        if video_info["status"] != "completed":
            status_message = (
                f"Video processing status: {video_info['status']}. Not yet completed."
            )
            logging.error(
                f"[{self.address}] Video not completed for retrieval: {video_id}. Status: {video_info['status']}"
            )
        ...
        if not processed_video_path or not os.path.exists(processed_video_path):
            logging.error(
                f"[{self.address}] Processed video file not found for {video_id} at {processed_video_path}"
            )
            await context.abort(
                grpc.StatusCode.INTERNAL, "Processed video file not found on master."
            )
            return
        ...
        try:
            logging.info(
                f"[{self.address}] Streaming processed video file {processed_video_path} for video ID: {video_id}"
            )
            ...
            with open(processed_video_path, "rb") as f:
                while True:
                    ...
                    if not chunk:
                        break
                    yield replication_pb2.RetrieveVideoChunk(
                        video_id=video_id, data_chunk=chunk
                    )

            logging.info(
                f"[{self.address}] Finished streaming processed video for video ID: {video_id}"
            )

        except Exception as e:
            logging.error(
                f"[{self.address}] Failed to stream processed video file for {video_id}: {type(e).__name__} - {e}",
                exc_info=True,
            )
            await context.abort(
                grpc.StatusCode.INTERNAL,
                f"Failed to stream processed video file: {type(e).__name__} - {e}",
            )

    async def GetVideoStatus(
        self,
        request: replication_pb2.VideoStatusRequest,
        context: grpc.aio.ServicerContext,
    ) -> replication_pb2.VideoStatusResponse:
        """Provides the processing status of a video (master only)."""
        if self.role != "master":
            return replication_pb2.VideoStatusResponse(
                video_id=request.video_id,
                status="not_master",
                message="This node is not the master and does not track video status.",
            )

        video_id = request.video_id
        logging.debug(
            f"[{self.address}] Received GetVideoStatus request for video ID: {video_id}"
        )
        ...
        if status in [
            "segmented",
            "shards_distributed",
            "all_shards_retrieved",
            "processing_failed",
            "concatenation_failed",
            "concatenation_prerequisites_failed",
        ]:
            total_shards = video_info.get("total_shards", 0)
            processed_count = sum(
                1
                for s in video_info["shards"].values()
                if s["status"] == "processed_successfully" or s["status"] == "retrieved"
            )
            retrieved_count = sum(
        ...
        return replication_pb2.VideoStatusResponse(
            video_id=video_id, status=status, message=message
        )

    async def ProcessShard(
        self,
        request: replication_pb2.DistributeShardRequest,
        context: grpc.aio.ServicerContext,
    ):
    ...
    async def _process_shard_background(self, request):
        ...
        try:
            ...
            container = (
                request.shard_id.split(".")[-1] if "." in request.shard_id else "mp4"
            )
            processed_output_filename = f"{request.shard_id}_processed.{container}"
            processed_output_temp_path = os.path.join(
                process_dir, processed_output_filename
            )
            processed_output_final_path = os.path.join(
                self.shards_dir, processed_output_filename
            )
            ...
            logging.error(
                f"[{self.address}] Error processing shard {request.shard_id}: {type(e).__name__} - {e}",
                exc_info=True,
            )
            await self._report_shard_status(
                request.video_id,
            ...
            if process_dir and os.path.exists(process_dir):
                await asyncio.get_event_loop().run_in_executor(
                    None, shutil.rmtree, process_dir
                )
    ...
    async def RequestShard(
        self,
        request: replication_pb2.RequestShardRequest,
        context: grpc.aio.ServicerContext,
    ) -> replication_pb2.RequestShardResponse:
        shard_id = request.shard_id
        ...
        logging.info(
            f"[{self.address}] RequestShard for {shard_id}, looking at {processed_fn}"
        )
        ...
        try:
            data = await asyncio.get_event_loop().run_in_executor(
                None, self._read_file_blocking, processed_path
            )

            return replication_pb2.RequestShardResponse(
                shard_id=shard_id, success=True, shard_data=data, message="OK"
            )
        except Exception as e:
    ...
    def _all_shards_processed_successfully(self, video_info):
        ...
        return all(
            status in ["processed_successfully", "retrieved"] for status in statuses
        )
    ...
    def _any_shard_failed(self, video_info):
        ...
        statuses = [s["status"] for s in video_info["shards"].values()]
        return any(
            status.startswith("failed") or status.endswith("retrieval_failed")
            for status in statuses
        )

    async def _report_shard_status(
        self, video_id: str, shard_id: str, status: str, message: str = ""
    ):
        """Reports the processing status of a shard to the master (worker only)."""
        master_stub = self._get_or_create_master_stub()
        if not master_stub:
            logging.error(
                f"[{self.address}] Cannot report shard status for {shard_id}. No master MasterService stub available. Storing as unreported."
            )
        ...
        try:
            logging.info(
                f"[{self.address}] Attempting to report status '{status}' for shard {shard_id} of video {video_id} to master {self.current_master_address} via MasterService stub"
            )
            response = await master_stub.ReportWorkerShardStatus(request)
            if response.success:
                logging.info(
                    f"[{self.address}] Successfully reported status for shard {shard_id}."
                )
                if (video_id, shard_id) in self._unreported_processed_shards:
                    del self._unreported_processed_shards[(video_id, shard_id)]
                    logging.info(
                        f"[{self.address}] Removed shard {shard_id} from unreported list after successful report."
                    )
            else:
                logging.error(
                    f"[{self.address}] Master rejected shard status report for {shard_id}: {response.message}. Storing as unreported."
                )
                self._unreported_processed_shards[(video_id, shard_id)] = status

        except grpc.aio.AioRpcError as e:
            logging.error(
                f"[{self.address}] RPC failed when reporting shard status for {shard_id} to master {self.current_master_address}: {e.code()} - {e.details()}. Storing as unreported.",
                exc_info=True,
            )
            self._unreported_processed_shards[(video_id, shard_id)] = status
        except Exception as e:
            logging.error(
                f"[{self.address}] Failed to report shard status for {shard_id} to master {self.current_master_address}: {type(e).__name__} - {e}. Storing as unreported.",
                exc_info=True,
            )
            self._unreported_processed_shards[(video_id, shard_id)] = status
    ...
    async def _attempt_report_unreported_shards(self):
        ...
        logging.info(
            f"[{self.address}] Attempting to report {len(self._unreported_processed_shards)} unreported processed shards to the new master {self.current_master_address}."
        )
    ...
    async def _check_other_nodes_health(self):
        ...
        while True:
            if self.role != "master":
                logging.info(
                    f"[{self.address}] No longer master, stopping health checks"
                )
                break
            ...
            for node_addr in list(self.known_nodes):
                ...
                if node_addr not in self._node_stubs:
                    try:
                        logging.info(
                            f"[{self.address}] Discovered new node {node_addr}, creating stubs"
                        )
                        self._create_stubs_for_node(node_addr)
                        logging.info(
                            f"[{self.address}] Stubs successfully created for {node_addr}"
                        )
                    except Exception as e:
                        logging.debug(
                            f"[{self.address}] Still can’t reach {node_addr}: {e}"
                        )
                ...
                try:
                    ...
                    logging.warning(
                        f"[{self.address}] Health check failed for {node_addr}: {rpc_err}"
                    )

                    if node_addr in self.known_nodes:
                        logging.info(
                            f"[{self.address}] Removing unreachable node {node_addr} from known_nodes"
                        )
                    ...
                    if node_addr in self._channels:
                        ...
                        try:
                            ...
                            logging.info(
                                f"[{self.address}] Closed channel to {node_addr}"
                            )
                        except Exception:
                            pass
                    ...
                    logging.error(
                        f"[{self.address}] Unexpected error checking {node_addr}: {exc}",
                        exc_info=True,
                    )
    ...
    async def start_election(self):
        ...
        # Recalculate and compare scores to defer if someone’s clearly better
        ...
        for node_addr in self.known_nodes:
            ...
            try:
                ...
                if resp.cpu_utilization < my_score:
                    better_nodes.append(node_addr)
            except Exception:
                continue

        if better_nodes:
            logging.info(f"[{self.address}] Deferring election: {len(better_nodes)} better-scoring nodes")
        ...
        # ask everyone except ourselves
        peers = [n for n in self.known_nodes if n != self.address and n != failed_master]
        tasks = []
        for peer in peers:
            stub = self._node_stubs.get(peer)
            if not stub:
                continue
            logging.info(f"[{self.address}] Sending vote request to {peer}")
            tasks.append((peer, asyncio.create_task(self._send_request_vote(stub, request, peer))))

        # if no peers, auto-win
        if not tasks:
            return await self._become_master()
        ...
        for (peer, _), result in zip(tasks, results):
            if isinstance(result, Exception) or result is None:
                logging.warning(f"[{self.address}] {peer} abstained (no reply)")
                continue
            responded += 1
            if result.vote_granted:
                self.votes_received += 1
                logging.info(f"[{self.address}] Vote granted by {peer}")

        # compute majority of (self + responded)
        total = 1 + responded
        majority = total // 2 + 1
        logging.info(f"[{self.address}] tally: {self.votes_received}/{total} (need {majority})")

        if self.votes_received >= majority:
            await self._become_master()
        else:
            self.state = "follower"
            # restart master-health check if needed
            if not self._master_health_check_task or self._master_health_check_task.done():
    ...
    def _validate_stub(self, node_addr: str) -> bool:
        ...
        try:
            ...
            return (
                channel.get_state(try_to_connect=True) == grpc.ChannelConnectivity.READY
            )
        except Exception:
            return False
    ...
    def get_best_nodes_by_score(self, count=1):
        ...
        sorted_nodes = sorted(
            self.node_scores.keys(), key=lambda addr: self.node_scores[addr].score
        )
    ...
    async def _become_master(self):
        ...
        if hasattr(self, "node_scores") and self.node_scores:
            ...
            if best_node != self.address and best_node in self.known_nodes:
                logging.info(
                    f"[{self.address}] Found better master candidate: {best_node}. Deferring leadership."
                )
                self.state = "follower"
                self.role = "worker"
        ...
        if (
            self._master_announcement_task is None
            or self._master_announcement_task.done()
        ):
            self._master_announcement_task = asyncio.create_task(
                self._master_election_announcement_routine()
            )
            self._background_tasks.append(self._master_announcement_task)
        if (
            self._other_nodes_health_check_task is None
            or self._other_nodes_health_check_task.done()
        ):
            self._other_nodes_health_check_task = asyncio.create_task(
                self._check_other_nodes_health()
            )
        ...
        if not restored:
            logging.info(
                f"[{self.address}] Remote backup restoration failed or not available. Checking local backup."
            )
            backup_dir = os.path.join(MASTER_DATA_DIR, "backup")
            if os.path.exists(backup_dir):
                logging.info(
                    f"[{self.address}] Found local backup directory, attempting to restore data"
                )
                try:
                    ...
                    for file in os.listdir(backup_dir):
                        ...
                        if os.path.isfile(src_path):
                            ...
                            logging.info(
                                f"[{self.address}] Restored local backup file: {file}"
                            )
                except Exception as e:
                    logging.error(
                        f"[{self.address}] Error restoring local backup data: {e}"
                    )
            else:
                logging.info(
                    f"[{self.address}] No local backup directory found at {backup_dir}"
                )

    async def _send_request_vote(
        self,
        node_stub: replication_pb2_grpc.NodeServiceStub,
        request: replication_pb2.VoteRequest,
        node_address: str,
    ) -> replication_pb2.VoteResponse:
        ...
        try:
            ...
            logging.info(
                f"[{self.address}] Received VoteResponse from {node_address}: {response.vote_granted}"
            )
            return response
        except (grpc.aio.AioRpcError, asyncio.TimeoutError) as e:
            logging.warning(
                f"[{self.address}] VoteRequest to {node_address} failed: {e}"
            )
            return None
        except Exception as e:
            logging.error(
                f"[{self.address}] Unexpected error sending VoteRequest to {node_address}: {e}",
                exc_info=True,
            )
            return None
    ...
    async def _master_election_announcement_routine(self):
        ...
        while self.role == "master":
            logging.info(
                f"[{self.address}] Announcing self as master (Term: {self.current_term})."
            )
            ...
            for node_addr in list(self._node_stubs):
                ...
                try:
                    ...
                    logging.warning(
                        f"[{self.address}] MasterAnnouncement to {node_addr} failed: {e}. Removing from cluster."
                    )
    ...
    async def _send_master_announcement(
        self, node_address: str, announcement: replication_pb2.MasterAnnouncement
    ):
        ...
        try:
            ...
            if channel:
                ...
                logging.debug(
                    f"[{self.address}] MasterAnnouncement sent successfully to {node_address}."
                )
            else:
                logging.warning(
                    f"[{self.address}] Could not create channel to {node_address} for MasterAnnouncement."
                )
        except (grpc.aio.AioRpcError, asyncio.TimeoutError) as e:
            logging.warning(
                f"[{self.address}] MasterAnnouncement to {node_address} failed: {e}"
            )
        except Exception as e:
            logging.error(
                f"[{self.address}] Error sending MasterAnnouncement to {node_address}: {e}",
                exc_info=True,
            )
    ...
    async def _start_election_with_delay(self):
        ...
        self.election_timeout = random.uniform(10, 15)
        logging.info(
            f"[{self.address}] Starting pre-election delay for {self.election_timeout:.2f} seconds."
        )
    ...
    async def _election_delay_coro(self):
        ...
        try:
            ...
            if self.state != "follower" or self.current_master_address:
                logging.info(
                    f"[{self.address}] Aborting election - cluster state changed during delay"
                )
                return

            if hasattr(self, "election_attempts") and self.election_attempts > 3:
                logging.warning(
                    f"[{self.address}] Detected potential election deadlock after {self.election_attempts} attempts"
                )

                for node_addr in sorted(self.known_nodes + [self.address]):
                    if node_addr == self.address:
                        logging.info(
                            f"[{self.address}] Forcing election resolution - becoming master by ID priority"
                        )
                        await self._become_master()
                        return
                    ...
                    if self._validate_stub(node_addr):
                        logging.info(
                            f"[{self.address}] Detected alive node {node_addr} with better ID priority"
                        )
                        break
    ...
    async def retry_register_with_master(self, interval: float = 5.0):
        ...
        while self.role == "worker":
            try:
                ...
                if resp.success:
                    logging.info(
                        f"[{self.address}] Successfully registered with master on retry: {resp.message}"
                    )
                    return
                else:
                    logging.info(
                        f"[{self.address}] Retry registration response: {resp.message}"
                    )
            except Exception as e:
    ...
    async def check_master_health(self):
        ...
        while self.role == "worker":
            if not self.current_master_address:
                logging.info(
                    f"[{self.address}] No master known, waiting briefly before checking again"
                )
                await asyncio.sleep(2)
                continue

            try:
                master_node_channel = self._get_or_create_channel(
                    self.current_master_address
                )
                if not master_node_channel:
                    logging.warning(
                        f"[{self.address}] Could not get channel to master at {self.current_master_address} for health check."
                    )
                    self.current_master_address = None
                    await self._start_election_with_delay()
                    continue

                master_node_stub = replication_pb2_grpc.NodeServiceStub(
                    master_node_channel
                )
                ...
                if response.known_nodes_count > len(self.known_nodes) + 1:
                    logging.info(
                        f"[{self.address}] Master knows more nodes than we do. Requesting update."
                    )
                    await self._request_node_list_update(master_node_stub)

                logging.debug(
                    f"[{self.address}] Master at {self.current_master_address} is healthy."
                )
                self.last_heartbeat_time = time.monotonic()
            except (grpc.aio.AioRpcError, asyncio.TimeoutError) as e:
                ...
                if time_since_last_heartbeat > self.election_timeout:
                    ...
                    self.current_master_address = None
                    logging.info(
                        f"[{self.address}] Init election after master {failed_master} removal"
                    )
                    await self._start_election_with_delay()

            except Exception as e:
                logging.error(
                    f"[{self.address}] Error checking master health: {e}", exc_info=True
                )
            await asyncio.sleep(2)
    ...
    async def _request_node_list_update(self, master_node_stub):
        ...
        try:
            response = await master_node_stub.GetAllNodes(
                replication_pb2.GetAllNodesRequest()
            )
            for node_info in response.nodes:
                node_addr = f"{node_info.address}:{node_info.port}"
                if node_addr != self.address and node_addr not in self.known_nodes:
                    logging.info(
                        f"[{self.address}] Adding newly discovered node {node_addr} to known_nodes"
                    )
                    self.known_nodes.append(node_addr)
                    self._create_stubs_for_node(node_addr)
        except Exception as e:
    ...
    async def _replicate_video_data(self, video_id: str, output_path: str):
        ...
        if not hasattr(self, "backup_servers") or not self.backup_servers:
            logging.warning(
                f"[{self.address}] No backup servers configured for replication"
            )
            return

        logging.info(
            f"[{self.address}] Starting replication of video {video_id} to backup servers"
        )
        ...
        for backup_addr in self.backup_servers:
            try:
                ...
                request = replication_pb2.ReplicateVideoRequest(
                    ...
                    container=os.path.splitext(output_path)[1][
                        1:
                ...
                if response.success:
                    logging.info(
                        f"[{self.address}] Successfully replicated video {video_id} to backup server {backup_addr}"
                    )
                else:
                    logging.error(
                        f"[{self.address}] Failed to replicate video {video_id} to backup server {backup_addr}: {response.message}"
                    )

            except Exception as e:
                logging.error(
                    f"[{self.address}] Error replicating to backup server {backup_addr}: {e}"
                )

    async def ReplicateVideo(
        self,
        request: replication_pb2.ReplicateVideoRequest,
        context: grpc.aio.ServicerContext,
    ) -> replication_pb2.ReplicateVideoResponse:
        ...
        try:
            ...
            logging.info(
                f"[{self.address}] Successfully replicated video {video_id} from master"
            )

            return replication_pb2.ReplicateVideoResponse(
                success=True, message="Video replicated successfully"
            )

        except Exception as e:
            logging.error(f"[{self.address}] Failed to replicate video: {e}")
            return replication_pb2.ReplicateVideoResponse(success=False, message=str(e))

    async def RequestBackupData(
        self,
        request: replication_pb2.RequestBackupDataRequest,
        context: grpc.aio.ServicerContext,
    ) -> replication_pb2.RequestBackupDataResponse:
        """Handles backup data requests from a new master."""
        try:
            backup_dir = os.path.join(MASTER_DATA_DIR, "backup")
            if not os.path.exists(backup_dir):
                return replication_pb2.RequestBackupDataResponse(
                    success=False, message="No backup directory found", backup_files=[]
                )
            ...
            logging.info(
                f"[{self.address}] Sending {len(backup_files)} backup files to new master"
            )
            return replication_pb2.RequestBackupDataResponse(
                success=True,
    ...
    async def _restore_from_backup_servers(self):
        ...
        if not hasattr(self, "backup_servers") or not self.backup_servers:
            logging.warning(
                f"[{self.address}] No backup servers configured for restoration"
            )
            return

        logging.info(
            f"[{self.address}] Attempting to restore data from {len(self.backup_servers)} backup servers"
        )
        ...
        for backup_addr in self.backup_servers:
            ...
            try:
                ...
                if response.success and response.backup_files:
                    logging.info(
                        f"[{self.address}] Received {len(response.backup_files)} backup files from {backup_addr}"
                    )

                    # Process the backup files
                    for backup_file in response.backup_files:
                        output_path = os.path.join(
                            MASTER_DATA_DIR, backup_file.filename
                        )
                        with open(output_path, "wb") as f:
                        ...
                        logging.info(
                            f"[{self.address}] Restored backup file for video {video_id}"
                        )

                    return True  # Successfully restored from this backup server
                else:
                    logging.warning(
                        f"[{self.address}] No backup files from {backup_addr}: {response.message}"
                    )

            except Exception as e:
                logging.error(
                    f"[{self.address}] Error restoring from backup server {backup_addr}: {e}"
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--port", type=int, required=True, help="Port to bind the server to"
    )
    parser.add_argument(
        "--role",
        type=str,
        ...
        required=True,
        help="Role of the node (master or worker)",
    )
    parser.add_argument(
        "--master",
        type=str,
        help="Address of the initial master node (host:port). Required for workers.",
    )
    parser.add_argument(
        "--nodes",
        type=str,
        nargs="*",
        default=[],
        help="List of known node addresses (host:port) in the network.",
    )
    parser.add_argument(
        "--backup-servers",
        type=str,
    ...
    if args.role == "worker" and args.master:
        if args.master not in args.nodes and args.master != node_address_arg:
            logging.info(
                f"[{node_address_arg}] Adding specified master {args.master} to list of connectable nodes for NodeServices."
            )
    ...
    try:
        ...
        logging.error(
            f"[{args.host}:{args.port}] Node execution failed: {type(e).__name__} - {e}",
            exc_info=True,
        )
    finally:
        if node_instance:
